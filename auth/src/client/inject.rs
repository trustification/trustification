...
pub trait TokenInjector: Sized + Send + Sync {
    async fn inject_token(self, token_provider: impl AsRef<dyn TokenProvider> + Send + Sync) -> Result<Self, Error>;
}
...
impl TokenInjector for reqwest::RequestBuilder {
    ...
    async fn inject_token(self, token_provider: impl AsRef<dyn TokenProvider> + Send + Sync) -> Result<Self, Error> {
        if let Some(credentials) = token_provider.as_ref().provide_access_token().await? {
