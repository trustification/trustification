...
During ingestion process, jobs can generate reports regarding warnings and errors encountered during the process.
The report is turned off by default. Tu turn it on, you need to enable the module and set authentication details im the
values file.
...
You can also provide these settings as parameters directly to the helm command

[source,bash]
----
helm upgrade --install -n $NAMESPACE trustification charts/trustification --values values-ocp-no-aws.yaml --set-string appDomain=$APP_DOMAIN --set image.registry=ghcr.io/dejanb --set image.version=latest --set image.pullPolicy=Always  --set modules.report.enabled=true --set report.auth.username=admin --set report.auth.password=admin123456
----

The reports then should be available at the URL of the appropriate route.
