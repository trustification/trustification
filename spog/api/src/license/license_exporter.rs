...
mod tests {
    ...
    async fn is_works_cydx() {
        ...
        let mut file =
            File::create("/tmp/application.cdx_licenses.zip").unwrap_or_else(|_| panic!("create file failed"));
    ...
    async fn is_works_spdx() {
        ...
        let export = LicenseExporter::new(sbom_licenses);
        let mut file = File::create("/tmp/mtv-2.6_licenses.zip").unwrap_or_else(|_| panic!("create file failed"));
        file.write_all(&export.generate().unwrap_or_else(|_| panic!("generate failed")))
            .unwrap_or_else(|_| panic!("write file failed"));
    }
}
