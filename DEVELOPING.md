...
```shell
curl -H "Transfer-Encoding: chunked" -X POST --json @bombastic/testdata/ubi9-sbom.json http://localhost:8082/api/v1/sbom?purl=ubi9
```
You can also post compressed SBOM's using the `Content-Encoding` header. Both `zstd` and `bzip2` encodings are supported:
```shell
curl -H "content-encoding: bzip2" -H "Transfer-Encoding: chunked" --json @openshift-4.13.json.bz2 http://localhost:8082/api/v1/sbom?purl=openshift-4.13
```
...
```shell
curl "http://localhost:8082/api/v1/sbom?purl=pkg%3Amaven%2Fio.seedwing%2Fseedwing-java-example%401.0.0-SNAPSHOT%3Ftype%3Djar"
curl -o ubi9-sbom.json "http://localhost:8082/api/v1/sbom?purl=ubi9"
```
