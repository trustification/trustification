...
```shell
curl -H "Transfer-Encoding: chunked" -X POST --json @bombastic/testdata/ubi9-sbom.json http://localhost:8082/api/v1/sbom?id=ubi9
```
You can also post compressed SBOM's using the `Content-Encoding` header. Both `zstd` and `bzip2` encodings are supported:
```shell
curl -H "content-encoding: bzip2" -H "Transfer-Encoding: chunked" --json @openshift-4.13.json.bz2 http://localhost:8082/api/v1/sbom?id=openshift-4.13
```
